...
public class PaymentService {
    ...
    @HystrixCommand(fallbackMethod = "paymentCircuitBreakersss_fallback",
    ...
    public String paymentCircuitBreakersss(@PathVariable("id") Integer id) {
        if (id < 0) {
            throw new RuntimeException("******id 不能为负数");
        }
        String serialNumber = IdUtil.simpleUUID();
    ...
    public String getCount() {
        ...
        redisScript.setResultType(String.class);//返回类型是String
        redisScript.setScriptSource(new ResourceScriptSource(new ClassPathResource("lua/list.lua")));
        Object result = redisTemplate.execute(redisScript, Arrays.asList("count1"));
